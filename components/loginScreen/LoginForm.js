...
const LoginForm = ({navigation}) => {
    ...
    const onLogin=async (email,password)=>{
        try{
            ...
            console.log("firebase successs",email,password)
        }catch(error){
            Alert.alert(
                'Nope',
                error.message+'\n',
  ...
  return (
    <View style={styles.wrapper}>
        <Formik initialValues={{email:'',password:''}}onSubmit={(values)=>{onLogin(values.email,values.password)}} validationSchema={loginFormSchema}
